...
/**
 * Chapter 10: jwk() (ES256 demo JWKS) + requireScopes on writes.
 * Production: replace DEMO_JWKS with your IdP https:// JWKS URL.
 */
export function buildApp(
  ...
  const app = new App({
    ...
    openapi: { info: { title: "orders-api", version: "0.10.0" } },
  ...
  app.post(
    ...
    async ({ body, state }) => {
      const user = state.user as { sub?: string; id?: string } | undefined;
      const ownerId = user?.sub ?? user?.id;
      if (!ownerId) throw new UnauthorizedError("missing user");
      const order = await createOrder({ ownerId, items: body.items });
  ...
  app.get(
    ...
    async ({ params, state }) => {
      ...
      const ownerId = user?.sub ?? user?.id;
      if (!ownerId) throw new UnauthorizedError("missing user");
      const order = await getOrderForOwner(params.id, ownerId);
