...
const RegisterWebhookSchema = z
  .object({
    url: z.string().min(1),
  })
  .strict();

const safeFetch = fetchGuard({ allowLoopback: false, allowPrivate: false });

/**
 * Chapter 12: Ch11 + webhook registration that probes the URL via fetchGuard.
 */
export function buildApp(
  ...
  const app = new App({
    ...
    openapi: { info: { title: "orders-api", version: "0.12.0" } },
  ...
  app.post(
    ...
    async ({ body }) => {
      try {
        // Probe with fetchGuard so metadata / private IPs never leave the box.
    ...
    (admin) => {
      admin.get(
        ...
        async () => ({
          ...
          body: {
            ok: true as const,
            note: "Admin summary (IP-restricted).",
          },
        }),
      );
    },
